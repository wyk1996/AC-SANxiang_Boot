...
uint8_t ch_is_under_volt(_GUN_NUM gun,CH_TASK_T *pstChTcb)
{
    ...
    {
        //===当前的时间减去第一次没有欠压的时间，例:第一次欠压后，你再启动第二次后，还会立马欠压，不会等到30s，因为时间无赋值。
        return 0;
    }
...
uint8_t ch_is_under_cp9v(CH_TASK_T *pstChTcb)
{
    ...
    if(rt_tick_get() - pstChTcb->stCP9Vstate.uiTick < (CH_TIME_60_SEC * 2))  //持续2分钟，返回
    {
        return 0;
    }
    return 1;
...
{
    if(ch_is_fault(&pstChTcb->stIOFault) || ch_is_over_current(gun,pstChTcb) || ch_is_over_volt(gun,pstChTcb) || ch_is_under_volt(gun,pstChTcb))
    {
        ch_jump_new_stat(pstChTcb,CHARGER_FAULT);
        return 1;
    }
...
uint8_t electricity_err(_GUN_NUM gun,CH_TASK_T *pstChTcb)
{
    static uint32_t  ChargeEnergybuf[2]={0},lasttime[2]={0},elect_err_flag[2]={0};
    if(CHARGING != pstChTcb->ucState)
    {
    ...
    if((dlt645_info[gun].out_cur > 1.0) || (pstChTcb->stRn8209.usCurrent > 100))  //内外电表大于1A时
    {
        ...
        {
            if(pstChTcb->stCh.uiAllEnergy != ChargeEnergybuf[gun])
            {
                ChargeEnergybuf[gun] = pstChTcb->stCh.uiAllEnergy;
                lasttime[gun] = rt_tick_get();
...
uint8_t ch_is_low_current(_GUN_NUM gun,CH_TASK_T *pstChTcb)
{
    ...
    if(rt_tick_get() - pstChTcb->stLowCurr.uiTick <= (CH_TIME_60_SEC * 5))  /*无电流5分*/
    {
        return 0;
    }
    return 1;
...
uint8_t ch_is_stp(_GUN_NUM gun,CH_TASK_T *pstChTcb)
{
	...
	if(electricity_err(gun,pstChTcb))
	{
...
//        }
    ...
    {
        ch_info_update(gun,pstChTcb);       //内部电表时，直接读取
    }
